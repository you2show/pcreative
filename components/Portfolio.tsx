...
const Portfolio: React.FC<PortfolioProps> = ({ showPopupOnMount = false, onPopupClose, usePathRouting = false }) => {
  ...
  return (
    <section id="portfolio" className="py-24 bg-gray-50/50 dark:bg-gray-900/50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <RevealOnScroll variant="fade-up" delay={400}>
          <div className="text-center mt-20">
             <button onClick={handleViewAllClick} className="px-10 py-4 rounded-full border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white font-bold hover:bg-white hover:text-gray-950 transition-all duration-300 font-khmer flex items-center gap-2 mx-auto active:scale-95">
      ...
      {isViewAllOpen && createPortal(
         <div className="fixed inset-0 z-[10001] overflow-hidden">
             <div className="relative w-full h-full bg-gray-50 dark:bg-gray-900 overflow-hidden animate-scale-up flex flex-col">
                <div className="flex justify-between items-center p-6 md:p-8 border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-gray-900 z-10">
                    <div>
                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white font-khmer">{t('All Projects', 'គម្រោងទាំងអស់')}</h3>
                        <p className="text-gray-600 dark:text-gray-400 text-sm font-khmer">{t('Browse our complete portfolio', 'មើលផលប័ត្រពេញលេញរបស់យើង')}</p>
                    </div>
                    <button onClick={handleViewAllClose} aria-label={t('Close', 'បិទ')} className="p-3 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-900 dark:text-white rounded-full transition-all border border-gray-100 dark:border-white/5 active:scale-95">
                        <X size={24} />
                    </button>
                </div>
                <div className="flex-1 overflow-y-auto p-6 md:p-8 scrollbar-hide">
                    <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                        {(projects || []).map((project) => (
                             <div key={project.id} onClick={() => { hapticTap(); openItem(project.slug || project.id); }} className="group relative rounded-xl overflow-hidden break-inside-avoid bg-gray-100 dark:bg-gray-800 border border-gray-100 dark:border-white/5 hover:border-indigo-500/30 transition-all duration-300 cursor-pointer active:scale-95">
                                ...
                                <div className="absolute inset-0 bg-white/80 dark:bg-gray-950/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center text-center p-4">
                                    <h4 className="text-gray-900 dark:text-white text-lg font-bold font-khmer">{project.title}</h4>
                                </div>
                            </div>
                        ))}
                    </div>
