                        ...
                        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                            <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                                <div className="space-y-1.5 md:space-y-2">
                                    <label htmlFor="contact-name" className="text-[10px] md:text-xs font-bold text-gray-500 ml-1 font-khmer uppercase tracking-wider">{t('Name', 'ឈ្មោះ')}</label>
                                    <input id="contact-name" name="name" value={formData.name} onChange={handleChange} type="text" required className="w-full px-4 py-3 md:py-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-khmer" placeholder="John Doe" />
                                </div>
                                <div className="space-y-1.5 md:space-y-2">
                                    <label htmlFor="contact-email" className="text-[10px] md:text-xs font-bold text-gray-500 ml-1 font-khmer uppercase tracking-wider">{t('Email', 'អ៊ីមែល')}</label>
                                    <input id="contact-email" name="email" value={formData.email} onChange={handleChange} type="email" required className="w-full px-4 py-3 md:py-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-khmer" placeholder="john@example.com" />
                                </div>
                            </div>
                            <div className="space-y-1.5 md:space-y-2">
                                <label htmlFor="contact-service" className="text-[10px] md:text-xs font-bold text-gray-500 ml-1 font-khmer uppercase tracking-wider">{t('Service', 'សេវាកម្ម')}</label>
                                <select id="contact-service" name="service" value={formData.service} onChange={handleChange} className="w-full px-4 py-3 md:py-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-khmer">
                            ...
                            <div className="space-y-1.5 md:space-y-2">
                                <label htmlFor="contact-message" className="text-[10px] md:text-xs font-bold text-gray-500 ml-1 font-khmer uppercase tracking-wider">{t('Message', 'សារ')}</label>
                                <textarea id="contact-message" name="message" value={formData.message} onChange={handleChange} rows={4} required className="w-full px-4 py-3 md:py-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-khmer resize-none" placeholder={t('Tell us about your project...', 'ប្រាប់យើងអំពីគម្រោងរបស់អ្នក...')}></textarea>
                            </div>
